...
async function handleApi(req, res, pathname) {
  ...
  if (req.method === "GET" && pathname === "/api/config") {
    return sendJson(res, 200, publicPricingConfig(pricing));
  }
  ...
  if (req.method === "POST" && pathname === "/api/estimate") {
    try {
      ...
      return sendJson(res, 200, calculateEstimate(payload, pricing));
    } catch (error) {
      return sendError(res, error);
    }
  }
  ...
  if (req.method === "POST" && pathname === "/api/quotes") {
    try {
      ...
      const payload = await readJsonBody(req);
      const estimate = calculateEstimate(payload.project, pricing);
