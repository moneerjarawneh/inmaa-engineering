(() => {
  ...
  async function initializeAuth() {
    try {
      const [sessionResponse, siteResponse] = await Promise.all([
        fetch("/api/auth/session", { headers: { Accept: "application/json" } }),
        fetch("/api/site-config", { headers: { Accept: "application/json" } })
      ]);
      const [result, config] = await Promise.all([parseResponse(sessionResponse), parseResponse(siteResponse)]);
      siteConfig = config;
      if (siteConfig.adminOrigin) adminLink.href = siteConfig.adminOrigin;
